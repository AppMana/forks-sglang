...
from sglang.srt.layers.attention.sparse_mla_triton.env import (
    triton_sparse_mla_head_block_size,
)


@triton.jit
def _e4m3_uint8_to_fp32(x_uint8):
    """Manual FP8 e4m3 -> FP32 unpack using only int/float ops.

    `tl.float8e4nv` requires sm_89+. On sm_86 (Ampere RTX 3090, A5000) we
    do the dequant in software with bit-twiddling on the raw uint8.

    e4m3 bit layout: [sign (1) | exp (4, biased 7) | mant (3)]. The format
    has no infinity; NaN is exp=0xF AND mant=0x7 (single encoding). Saturated
    max finite = ±448 (S100 1.110 = 1.75 * 2^7 = 224 * 2 — wait, max finite
    is exp=0xE mant=0x7 = 1.875 * 2^7 = 240; with exp=0xF mant<=0x6 also
    finite up to 448 in the "extended" e4m3 used by NVIDIA tensor cores).
    For the V4 KV cache we use the IEEE-754-style subnormal+normal form.
    """
    sign = ((x_uint8 >> 7) & 1).to(tl.int32)
    exp = ((x_uint8 >> 3) & 0xF).to(tl.int32)
    mant = (x_uint8 & 0x7).to(tl.int32)
    sign_mul = 1.0 - 2.0 * sign.to(tl.float32)
    mant_f = mant.to(tl.float32) * 0.125
    is_subnormal = exp == 0
    fp32_normal = sign_mul * tl.exp2((exp - 7).to(tl.float32)) * (1.0 + mant_f)
    fp32_subnormal = sign_mul * tl.exp2(-6.0) * mant_f
    return tl.where(is_subnormal, fp32_subnormal, fp32_normal)
...
def _accumulate_fp8ds_global_slots_attention_chunk_kernel(
    ...
    for candidate_idx in range(0, num_candidates):
        ...
        if is_valid:
            ...
            x_float = _e4m3_uint8_to_fp32(x_uint8)
